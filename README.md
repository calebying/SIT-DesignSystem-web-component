...
The CDN loader registers all Canvas elements up front. Depending on your usage, you may or may not need to load the polyfill.
...
<script src="https://cdn.jsdelivr.net/npm/@sit-canvas/canvas-web-component@<version>"></script>

//or load a single component e.g. Masthead
<script src="https://cdn.jsdelivr.net/npm/@sit-canvas/canvas-web-component@<version>/components/Masthead/index.umd.js"></script>

```

## Local Installation

You can also install Canvas web components locally with the following command

```js

npm install @sit-canvas/canvas-web-component @webcomponents/scoped-custom-element-registry
...
| ---------------------------- | ---------------------------------------------------------- |
| --sit-body-font-family      | Set the font family of the web components                  |
| --sit-body-font-size        | Set the font size of the web components                    |
| --sit-body-font-weight      | Set the font weight of the web components                  |
| --sit-body-line-height      | Set the line height of the web components                  |
| --sit-{stateColor}-rgb      | State colors in red,green,blue value                       |
| --sit-{stateColor}          | State colors in hexadecimal value                          |
| --sit-{stateColor}          | State colors in hexadecimal value                          |
| --sit-{stateColor}-{weight} | State colors with different weightage in hexadecimal value |
| --sit-gray-{weight}         | Gray colors with different weightage in hexadecimal value  |
...
```css
:root {
  --sit-body-font-family: "Inter";
  --sit-body-font-size: 5rem;
  --sit-primary-rgb: 89, 37, 220;
  --sit-secondary: #1f69ff;
  --sit-success-500: #3bb346;
  --sit-gray-500: #667085;
}
```
...
Canvas web component library is shipped with Canvas v2 stylings and does not require you to configure or install any other styling files.
...
Canvas web component library is shipped with our v2 design library, a spin-off from Bootstrap v5. As such, the css tokens are applicable to use and exposed to the shadow DOM.

For example, you can pass "btn-lg" and "me-2" to `buttonClasses` prop as another way to customise the stylings of your button.

```html
<sit-button buttonClasses="btn-lg me-2">Hello world</sit-button>
```
...
Any external stylings done on our web components like positioning needs to be done on your end. You can use [Canvas v2 library](https://github.com/calebying/SIT-DesignSystem-web-componentget-started/)to leverage on the position stylings we provide e.g. ms-auto, flexbox, grids
...
## Referencing sit-web-components in Angular

```typescript
//alert.component.ts
import { Component, ElementRef, ViewChild } from '@angular/core';
import { SitAlert } from '@sit-canvas/canvas-web-component';
...
<sit-button (click)="showAlert()">{{buttonText}}</sit-button>
<sit-alert #alert [show]="alertShowState">Alerting</sit-alert>
...
import { SitTooltip } from "@sit-canvas/canvas-web-component/react/tooltip";
```
  ...
  chainWebpack: config => {
    config.module
      ...
      .tap(options => ({
        ...options,
        compilerOptions: {
          // treat any tag that starts with sit- as custom elements
          isCustomElement: tag => tag.startsWith("sit-")
        }
      }));
  }
};
...
<template>
<div>
<sit-footer :title="footerAttr.title" :description="footerAttr.description" :lastUpdatedDate="footerAttr.date" .:links="footerAttr.links"></sit-footer>
</div>
</template>
...
```typescript
// e.g. of v-model usage on sit web components
// e.g. using sit events for callbacks usage
<template>
<form>
<sit-input @sit-input="onInput" v-model="inputValue" placeholder="Enter your name">
    </sit-input>
...
<sit-sidenav>
  <sit-sidenav-item>
    ...
    <sit-sidenav-link>sit-sidenav-link (control by Argstable)</sit-sidenav-link>
    <sit-sidenav-link href="#" disabled="">
      sit-sidenav-link
    </sit-sidenav-link>
...
Whenever a sit component uses composition (meaning it uses another sit component inside), we apply ScopedElementsMixin to make sure it uses the right version of this internal component.

For users who are using sit component directly for builing application, use the custom elements directly by [importing the custom elements](#using-the-custom-elements)

For users who are building component libraries on top of sit-web-component, please adopt OpenWC's scoped elements to prevent exporting our registered custom elements.
...
<sit-alert show variant="success"><svg slot="icon" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-circle" viewBox="0 0 16 16">
<path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
<path d="M7.002 11a1 1 0 1 1 2 0 1 1 0 0 1-2 0zM7.1 4.995a.905.905 0 1 1 1.8 0l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 4.995z"/>
</svg><small>we are live and stable now! Please re-read the documentation on how to import the components if you were previously using our unstable version (i.e. package version less than 1.0.0)</small></sit-alert>

# canvas-web-component

[![npm](https://img.shields.io/npm/dw/@sit-canvas/canvas-web-component?label=npm&style=flat-square)](https://www.npmjs.com/package/@sit-canvas/canvas-web-component)
[![jsDelivr](https://data.jsdelivr.com/v1/package/npm/@sit-canvas/canvas-web-component/badge)](https://www.jsdelivr.com/package/npm/@sit-canvas/canvas-web-component)
[![github](https://img.shields.io/badge/GitHub-Code-232323.svg?style=flat-square&logo=github&logoColor=white)](https://github.com/calebying/SIT-DesignSystem-web-component)

<div class="home-card-container mt-5">
    <sit-card class="col">
        <span slot="title">Canvas v2+ <i class="bi bi-filetype-scss"></i></span>
        <a slot="link" href="https://github.com/calebying/SIT-DesignSystem-web-component" target="_blank"></a>
        <p slot="description">The components are shipped with Canvas v2 styles and can be used straight out of the box.</p>
    </sit-card>
     <sit-card class="col">
