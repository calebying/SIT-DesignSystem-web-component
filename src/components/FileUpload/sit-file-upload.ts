...
export class SitFileUpload extends SitFormValidatorMixin(FormControlElement) {
  ...
  private _setFileList(files: FileList, previousCount = 0, deletedFile?: File) {
    ...
    this.emit<ISitFileUploadChangeEventDetail>("sit-change", { detail: files });

    // Always emit sit-files-selected for backwards compatibility (deprecated)
    this.emit<ISitFileUploadFilesSelectedEventDetail>("sit-files-selected", { detail: files });
    ...
    if (files.length > previousCount) {
      ...
      this.emit<ISitFileUploadAddFilesEventDetail>("sit-add-files", { detail: dt.files });
    ...
    if (files.length < previousCount && deletedFile) {
      this.emit<ISitFileUploadRemoveFileEventDetail>("sit-remove-file", { detail: { file: deletedFile, files } });
    }
  }
  ...
  protected _handleBlur() {
    ...
    if (this._mixinShouldSkipSitValidation()) return;
    if (sitBlur.defaultPrevented) return;
        ...
        return html`
          <li class="file-upload-list-item-container ${this.exitingFile === file ? "file-upload-exit" : ""}">
            <div class="file-upload-list-item ${metadata?.error ? "file-upload-error" : ""}">
              ${metadata?.uploading ? html`<sit-spinner size="sm"></sit-spinner>` : getCheckedIcon(metadata)}
