...
Always label your form components.

Use `label` prop in `Sit-radio-group` to provide a context for the entire radio group.
Add labels to individual `Sit-radio` through its default slot 
...
`Sit-radio-group` controls and updates the `invalid` prop in its child `Sit-radio` buttons.
...
Disabled individual `Sit-radio` using the `disabled` prop.
...
- To enable invalid styles and feedback message, set `hasFeedback` to true on `Sit-radio-group`.
