...
- Use `info` alert to provide additional information to users that is related to their completion of current action or task. Info variant uses the primary brand colour.
...
Use `<sit-alert-link>` for links within the alert
...
Use the `action` slot to place a call-to-action button inside a filled alert. Pass `<sit-button>` with `variant="outline"` and `size="sm"`. Use `tone="fixed-light"` for all variants except `warning`, which uses `tone="fixed-dark"` to maintain contrast against the light background.

<Canvas of={AlertStories.FilledWithAction}>
  <Story of={AlertStories.FilledWithAction} />
</Canvas>

## Action slot (outlined)

For outlined alerts, use `<sit-button>` with `variant="outline"`, `size="sm"`, and `tone="neutral"`.

<Canvas of={AlertStories.OutlinedWithAction}>
  <Story of={AlertStories.OutlinedWithAction} />
</Canvas>