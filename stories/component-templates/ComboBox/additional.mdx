...
Populate menu list of the combo box with `<sit-combo-box-option>`.
...
`SitComboBox` contains the `filterFunction` prop that is called on each item's label in the `menuList` whenever the input value changes, to determine whether an item should be displayed in the dropdown menu based on the input value.
...
```
const comboBox = document.querySelector('Sit-combo-box');
...
## Access input's display value via custom event: Sit-input

Hook onto the custom event: `Sit-input` to obtain the display value of the input
...
```ts
import type { ISitComboBoxInputEventDetail } from "@sit-canvas/canvas-web-component/components/ComboBox/sit-combo-box";

const inputHandler = (e: CustomEvent<ISitComboBoxInputEventDetail>) => {
...
Set the `async` attribute to true. You can then hook onto the `Sit-input` event to listen for input changes and make API requests accordingly.
...
Set `required` and `hasFeedback` to enable constraint validation with feedback styling.

<Canvas of={ComboBoxStories.Validation}>
...
<Canvas of={ComboBoxStories.OverrideInvalidFeedback}>
  <Story of={ComboBoxStories.OverrideInvalidFeedback} />
</Canvas>
...
Use `scrollBottomOffset` (number, default `0`) to configure how many pixels before the bottom of the menu the `Sit-scroll-end` event fires. This is useful for implementing infinite scroll or lazy-loading more options as the user scrolls toward the end of the list.
