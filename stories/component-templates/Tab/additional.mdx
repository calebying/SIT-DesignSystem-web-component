...
The variants of tab is controlled by `SitTabGroup`
...
The density of tabs is controlled by `SitTabGroup`. For a more compact tabs, modify `density` prop to `compact`
...
The orientation of tabs is controlled by `SitTabGroup`.
...
The `Sit-tab-group` component emits custom events when tabs are shown or hidden, allowing you to perform side effects and track which tab is currently active.

### Available events

- **`Sit-tab-show`**: Emitted when a tab becomes active. Use `event.detail.name` to get the active tab's panel name.
- **`Sit-tab-hide`**: Emitted when a tab is hidden (another tab becomes active). Use `event.detail.name` to get the hidden tab's panel name.
