...
Add `noValidate` to disable Sit built-in validation, then listen to `Sit-change-date` and call `setInvalid(bool)` with a matching `invalidFeedback` to drive the error state from your own logic.
...
**This behaviour is distinct from form validation.** It is a built-in input sanitisation mechanism that works regardless of whether `noValidate` is applied. Even when you opt out of Sit constraint validation with `noValidate`, the datepicker will still clear impossible dates on blur.
...
The `Sit-invalid` event is emitted when an invalid date is detected. The input value is then cleared on blur. You can detect the clear by checking if the value is empty in your `Sit-blur` handler.
