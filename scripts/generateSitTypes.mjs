/**
 * generateSitTypes.mjs
 *
 * Auto-generates sit-types.d.ts from custom-elements.json
 * and per-component lib/components/<Name>/types.d.ts files.
 *
 *   sit-types.d.ts  – React JSX IntrinsicElements (event props: "onsit-*")
 *
...
const sitComponents = getSitComponents(allComponents);
...
function buildPropInterfaceSection() {
  ...
  for (const comp of sitComponents) {
    ...
    if (!tagName?.startsWith("sit-")) continue;
    ...
    for (const ev of events) {
      ...
      if (ev.detailType) {
        ...
        propLines.push(`  ${kebabKey}?: SitEventHandler;`);
        propLines.push(`  ${camelKey}?: SitEventHandler;`);
      }
    }

    lines.push("");
    lines.push(`// ── ${className.replace(/^Sit/, "")} ─────────────────────────────────────────────────────────────`);
    lines.push("");
    lines.push(`export interface ${propsName} extends SitBaseProps {`);
 ...
 */`);
  ...
  lines.push("// ---------------------------------------------------------------------------");
  lines.push("");
  lines.push("export type SitEventHandler = (event: CustomEvent) => void;");
  lines.push("");
  lines.push("/** Common props shared by every Canvas element */");
  lines.push("export interface SitBaseProps extends React.HTMLAttributes<HTMLElement> {");
...
function generateReactWrapperTypes() {
  ...
  for (const comp of sitComponents) {
    ...
    const tagWithoutPrefix = tagName.replace(/^sit-/, "");
    const propsName = toPropsInterface(className);
