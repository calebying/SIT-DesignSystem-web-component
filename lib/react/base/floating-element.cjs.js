 ...
 */
class FloatingElement extends sitElement["default"] {
    constructor() {
        super(...arguments);
    ...
    defaultMiddleware() {
        return [dom.offset(8), dom.flip(), dom.shift()];
    }
    async updateFloatingPosition() {
        ...
        const { x, y, strategy, placement } = await dom.computePosition(this.referenceRef.value, this.floatingRef.value, opts);
    ...
    startFloatingAutoUpdate() {
        if (!this.referenceRef.value || !this.floatingRef.value)
            return;
        this._cleanupAutoUpdate = dom.autoUpdate(this.referenceRef.value, this.floatingRef.value, () => this.updateFloatingPosition());
    }
...
//# sourceMappingURL=floating-element.cjs.js.map
