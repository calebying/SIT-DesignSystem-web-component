 ...
 */
class DropdownElement extends sitElement["default"] {
    constructor() {
        ...
        this.dropdownMenuId = generateId["default"]("dropdown-menu", "div");
    ...
    _startAutoUpdate() {
        if (this.myDropdown.value && this.menuRef.value) {
            this._cleanupAutoUpdate = dom.autoUpdate(this.myDropdown.value, this.menuRef.value, () => this.updateFloatingPosition());
        }
    }
    ...
    async updateFloatingPosition() {
        ...
        const defaultMiddleware = [dom.offset(8), !this.noFlip ? dom.flip() : undefined, dom.shift()].filter(Boolean);
        ...
        const { x, y, strategy, placement: computedPlacement } = await dom.computePosition(this.myDropdown.value, this.menuRef.value, opts);
