...
class DatepickerCalendar extends sitElement["default"] {
    constructor() {
        super(...arguments);
        /** @internal */
        this.selectedDate = [];
    ...
    _generateIncrementDates() {
        const start = time.setTimeToNoon(this.selectedDate[0]);
        if (this.selectedDate.length < 2) {
            return [start];
        }
        const end = time.setTimeToNoon(this.selectedDate[1]);
        const arr = [];
    ...
    _onClickDay(event) {
        ...
        if (event.type === "click") {
            this.focusedDate = time.setTimeToNoon(new Date(date));
        }
        if (this.mode === "single") {
        ...
        const minimumDate = this.minDate ? time.setTimeToNoon(new Date(this.minDate)) : null;
        const maximumDate = this.maxDate ? time.setTimeToNoon(new Date(this.maxDate)) : null;
        const selectedDate = time.setTimeToNoon(displayDateClone);
    ...
    _generateDays() {
        const selectedDates = this.selectedDate.map(d => time.setTimeToNoon(d));
        ...
        for (let i = 0; i < 9; i++) {
            ...
            for (let j = 0; j <= 6; j++) {
                if (day <= monthLength && (i > 0 || j >= startingDayOfMonth)) {
                    ...
                    const ariaLabel = `${isCurrentDay && isCurrentMonth && isCurrentYear ? "Today's date, " : ""}` + dateFns.format(dateObj, "PPPP");
                    week.push(lit.html `<td
              key=${j}
              data-date=${dateStr}
              data-day=${day}
              aria-label=${ariaLabel}
              aria-current=${ifDefined_js.ifDefined(isCurrentDay && isCurrentMonth && isCurrentYear ? "date" : undefined)}
              ...
              ?disabled=${beforeMinDate || afterMinDate}
              aria-disabled=${ifDefined_js.ifDefined(beforeMinDate || afterMinDate ? "true" : undefined)}
              role="gridcell"
            >
              ${day}
            </td>`);
                    ...
                    week.push(lit.html `<td key=${j}></td>`);
                }
            }
            weeks.push(lit.html `<tr key=${i}>
          ${week}
        </tr>`);
            if (day > monthLength) {
                break;
        ...
        const dayView = lit.html `
      <table role="grid">
        <thead>
          <tr>
            ${DatepickerCalendar.DAY_LABELS.map((label, index) => lit.html ` <th key=${index} abbr=${label} scope="col">${label.slice(0, 3)}</th> `)}
          </tr>
        </thead>
        <tbody>
          ${weeks}
    ...
    _generateMonths() {
        ...
        const selectedTime = rangeDates.map(e => time.setTimeToNoon(new Date(e.getFullYear(), e.getMonth())).getTime());
        const year = this.displayDate.getFullYear();
        const monthView = lit.html `
      <div class="monthpicker" role="grid">
        ${[0, 1, 2, 3].map(row => lit.html `
            <div role="row">
            ...
            const time$1 = time.setTimeToNoon(new Date(year, idx)).getTime();
            ...
            return lit.html ` <button
                  role="gridcell"
                  class=${classMap_js.classMap({
                active: selectedTime.includes(time$1),
                ...
                "selected-ends": (isFirstSelectedMonth && isFirstSelectedYear) || (isLastSelectedMonth && isLastSelectedYear)
                  ...
                  data-year=${year}
                  tabindex="0"
                  aria-selected=${selectedTime.includes(time$1) ? "true" : "false"}
                  aria-label=${ariaLabel}
                >
                  ${m.slice(0, 3)}
                </button>`;
    ...
    _generateYears() {
        ...
        const yearArray = time.createYearViewArray(this.displayDate, CURRENT_YEAR);
        const yearView = lit.html `
        ...
        ${[0, 1, 2, 3].map(row => lit.html `
            ...
            return lit.html `
                ...
                "selected-ends": isFirstSelectedYear || isLastSectedYear
                    ...
                    aria-label=${ifDefined_js.ifDefined(CURRENT_YEAR === y ? `Current year, ${y}` : undefined)}
                  >
                    ${y}
                  </button>
                `;
    ...
    render() {
        ...
        switch (this.view) {
            case "days":
                viewContent = lit.html ` ${this._generateDays()} `;
                break;
            case "months":
                viewContent = lit.html ` ${this._generateMonths()} `;
                break;
            case "years":
                viewContent = lit.html ` ${this._generateYears()} `;
                break;
            default:
                viewContent = lit.html ` ${this._generateDays()} `; // Set a default view
                break;
        }
        return lit.html ` <div class="datepicker-body">${viewContent}</div> `;
    }
}
DatepickerCalendar.styles = [datepickerCalendar["default"]];
/** @internal */
...
//# sourceMappingURL=datepicker-calendar.cjs.js.map
