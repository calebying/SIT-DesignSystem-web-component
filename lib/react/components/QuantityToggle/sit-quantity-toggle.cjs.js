'use client';
'use strict';

Object.defineProperty(exports, '__esModule', { value: true });

var tslib = require('tslib');
var decorators_js = require('lit/decorators.js');
var classMap_js = require('lit/directives/class-map.js');
var ifDefined_js = require('lit/directives/if-defined.js');
var live_js = require('lit/directives/live.js');
var staticHtml_js = require('lit/static-html.js');
var formControlElement = require('../../base/form-control-element.cjs.js');
var formPlaceholder = require('../../styles/form-placeholder.cjs.js');
var svg = require('../../styles/svg.cjs.js');
var defaultvalue = require('../../utils/defaultvalue.cjs.js');
var validatorMixin = require('../../utils/validatorMixin.cjs.js');
var sitIconButton = require('../IconButton/sit-icon-button.cjs.js');
var sitInput = require('../Input/sit-input.cjs.js');
var quantityToggle = require('./quantity-toggle.cjs.js');
var formTextControl = require('../../styles/form-text-control.cjs.js');

/**
 * @summary The quantity toggle component is used to increase or decrease an incremental venue,  best used when the user needs to enter or adjust the quantity of a selected item.
 *
 * @slot invalidIcon - The slot for invalid icon
 *
 * @event sit-change - Emitted when an alteration to the control's value is committed by the user.
 * @event sit-input - Emitted when the control receives input and its value changes.
 *
 */
class SitQuantityToggle extends validatorMixin.SitFormValidatorMixin(formControlElement["default"]) {
    constructor() {
        super(...arguments);
        /** The input's value. Set to 0 by default */
        this.value = 0;
        /**  Controls the incremental / decremental value of the input */
        this.step = 1;
        /** Sets the quantity toggle as readonly  */
        this.readonly = false;
        /** Gets or sets the default value used to reset this element. The initial value corresponds to the one originally specified in the HTML that created this element. */
        this.defaultValue = 0;
    }
    /**
     * Checks for validity. Under the hood, HTMLFormElement's reportValidity method calls this method to check for component's validity state
     * Note that the native error popup is prevented for Canvas form components by default. Instead the validation message shows up in the feedback container of SitInput
     */
    reportValidity() {
        return this._mixinReportValidity();
    }
    /**
     * Checks for validity without any native error popup message
     */
    checkValidity() {
        return this._mixinCheckValidity();
    }
    /**
     * Returns the ValidityState object
     */
    get validity() {
        return this._mixinGetValidity();
    }
    /**
     * Returns the validation message based on the ValidityState
     */
    get validationMessage() {
        return this._mixinGetValidationMessage();
    }
    async _handleChange() {
        const sitInput = await this._sitInput;
        if (parseInt(sitInput.value) < this.step || sitInput.value === "") {
            sitInput.value = "0";
        }
        this.value = parseInt(sitInput.value);
        this._mixinSetFormValue();
        this._mixinValidate(sitInput.input);
        this.invalid = !this._mixinReportValidity();
    }
    async _handleInputChange() {
        const sitInput = await this._sitInput;
        this.invalid = false;
        if (parseInt(sitInput.value) < this.step || sitInput.value === "") {
            sitInput.value = "0";
        }
        this.value = parseInt(sitInput.value);
        this._mixinSetFormValue();
        this._mixinValidate(sitInput.input);
    }
    async _mixinResetFormControl() {
        const sitInput = await this._sitInput;
        this.value = this.defaultValue;
        sitInput.input.value = this.value.toString();
        this._mixinResetValidity(sitInput.input);
    }
    _handleKeyDown(event) {
        const allowedKeys = [
            "Backspace",
            "ArrowUp",
            "ArrowDown",
            "ArrowLeft",
            "ArrowRight",
            ...Array.from(Array(10).keys()).map(key => key.toString()),
            "Tab"
        ];
        // Allow keydown event only if the pressed key is in the allowedKeys array
        if (!allowedKeys.includes(event.key)) {
            event.preventDefault();
        }
    }
    _handleInvalid() {
        this.invalid = true;
    }
    _handleValid() {
        this.invalid = false;
    }
    /** Simulates a click on the plus button */
    plus() {
        this.plusBtn.click();
    }
    /** Simulates a click on the minus button */
    minus() {
        this.minusBtn.click();
    }
    async _onPlus(event) {
        const sitInput = await this._sitInput;
        event.preventDefault();
        event.stopPropagation();
        this.value = parseInt(sitInput.value) + parseInt(sitInput.step.toString());
        this._validateOnClick(sitInput.input);
    }
    async _onMinus(event) {
        const sitInput = await this._sitInput;
        event.preventDefault();
        event.stopPropagation();
        if (this.value < this.step) {
            this.value = 0;
        }
        else {
            this.value = parseInt(sitInput.value) - parseInt(sitInput.step.toString());
        }
        this._validateOnClick(sitInput.input);
    }
    /**
     * Validates the input on button clicks of the toggle.
     * Input is validated every time the button is click to update the invalid state
     * to indiciate the validity of quantity toggle
     * @param input native HTMLInputElement
     */
    async _validateOnClick(input) {
        const sitInput = await this._sitInput;
        await sitInput.updateComplete;
        this._mixinSetFormValue();
        this._mixinValidate(input);
        this.invalid = !this._mixinReportValidity();
    }
    _renderFeedback() {
        const wantFeedbackText = this.hasFeedback === "both" || this.hasFeedback === "text";
        return this.invalid && wantFeedbackText
            ? staticHtml_js.html ` <div class="invalid-feedback-container">
          <slot name="invalidIcon">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
              <path
                d="M17.5 10C17.5 14.1421 14.1421 17.5 10 17.5C5.85786 17.5 2.5 14.1421 2.5 10C2.5 5.85786 5.85786 2.5 10 2.5C14.1421 2.5 17.5 5.85786 17.5 10ZM10 6.25C9.49805 6.25 9.10584 6.68339 9.15578 7.18285L9.48461 10.4711C9.51109 10.7359 9.7339 10.9375 10 10.9375C10.2661 10.9375 10.4889 10.7359 10.5154 10.4711L10.8442 7.18285C10.8942 6.68339 10.5019 6.25 10 6.25ZM10.0014 11.875C9.48368 11.875 9.06394 12.2947 9.06394 12.8125C9.06394 13.3303 9.48368 13.75 10.0014 13.75C10.5192 13.75 10.9389 13.3303 10.9389 12.8125C10.9389 12.2947 10.5192 11.875 10.0014 11.875Z"
                fill="currentColor"
              />
            </svg>
          </slot>
          <div id="${this._controlId}-invalid" class="invalid-feedback">
            ${this.invalidFeedback ? this.invalidFeedback : this.validationMessage}
          </div>
        </div>`
            : staticHtml_js.html `${this._renderHintText()}`;
    }
    _renderLabel() {
        const labelTemplate = staticHtml_js.html `
      <label
        for=${this._controlId}
        id=${this._labelId}
        class=${classMap_js.classMap({
            "form-label": true,
            disabled: this.disabled
        })}
        >${this.label}</label
      >
    `;
        return this.label && labelTemplate;
    }
    _renderHintText() {
        const hintTextTemplate = staticHtml_js.html ` <div id="${this._controlId}Help" class="form-text">${this.hintText}</div> `;
        return this.hintText && hintTextTemplate;
    }
    render() {
        return staticHtml_js.html `
      <div class="form-control-container m-width-160">
        ${this._renderLabel()}
        <div class="input-group">
          <sit-icon-button
            class="minus-btn"
            variant="outline"
            tone="neutral"
            size="md"
            ariaLabel=${`decrease by ${this.step}`}
            ?disabled=${this.disabled ||
            (this.min !== undefined ? this.value <= this.min : this.value < 1) ||
            this.readonly}
            @click=${this._onMinus}
            name="dash"
          >
          </sit-icon-button>
          <sit-input
            type="number"
            class="quantity-toggle"
            name=${ifDefined_js.ifDefined(this.name)}
            step=${ifDefined_js.ifDefined(this.step)}
            min=${ifDefined_js.ifDefined(this.min)}
            max=${ifDefined_js.ifDefined(this.max)}
            .value=${live_js.live(this.value)}
            @sit-change=${this._handleChange}
            @sit-input=${this._handleInputChange}
            @sit-invalid=${this._handleInvalid}
            @sit-valid=${this._handleValid}
            @keydown=${this._handleKeyDown}
            ?disabled=${this.disabled}
            id=${this._controlId}
            ?invalid=${this.invalid}
            hasFeedback=${ifDefined_js.ifDefined(this.hasFeedback !== "text" ? "style" : undefined)}
          ></sit-input>
          <sit-icon-button
            class="plus-btn"
            variant="outline"
            tone="neutral"
            size="md"
            ariaLabel=${`increase by ${this.step}`}
            @click=${this._onPlus}
            ?disabled=${this.disabled ||
            (this.max !== undefined && this.max && this.value >= this.max) ||
            this.readonly}
            name="plus"
          >
          </sit-icon-button>
        </div>
        <div id="${this._controlId}-announcer" aria-live="assertive" class="visually-hidden">${this.value}</div>
        ${this._renderFeedback()}
      </div>
    `;
    }
}
SitQuantityToggle.styles = [
    ...formControlElement["default"].styles,
    formPlaceholder["default"],
    svg["default"],
    formTextControl["default"],
    quantityToggle["default"]
];
/** @internal */
SitQuantityToggle.dependencies = {
    "sit-input": sitInput.SitInput,
    "sit-icon-button": sitIconButton.SitIconButton
};
tslib.__decorate([
    decorators_js.query("sit-icon-button[ariaLabel^='increase by']")
], SitQuantityToggle.prototype, "plusBtn", void 0);
tslib.__decorate([
    decorators_js.query("sit-icon-button[ariaLabel^='decrease by']")
], SitQuantityToggle.prototype, "minusBtn", void 0);
tslib.__decorate([
    decorators_js.property({ type: Number, reflect: true })
], SitQuantityToggle.prototype, "value", void 0);
tslib.__decorate([
    decorators_js.property({ type: Number })
], SitQuantityToggle.prototype, "step", void 0);
tslib.__decorate([
    decorators_js.property({ type: Number })
], SitQuantityToggle.prototype, "min", void 0);
tslib.__decorate([
    decorators_js.property({ type: Number })
], SitQuantityToggle.prototype, "max", void 0);
tslib.__decorate([
    decorators_js.property({ type: String, reflect: true })
], SitQuantityToggle.prototype, "hasFeedback", void 0);
tslib.__decorate([
    decorators_js.property({ type: String, reflect: true })
], SitQuantityToggle.prototype, "invalidFeedback", void 0);
tslib.__decorate([
    decorators_js.property({ type: Boolean, reflect: true })
], SitQuantityToggle.prototype, "readonly", void 0);
tslib.__decorate([
    defaultvalue.defaultValue()
], SitQuantityToggle.prototype, "defaultValue", void 0);
tslib.__decorate([
    decorators_js.queryAsync("sit-input")
], SitQuantityToggle.prototype, "_sitInput", void 0);

exports.SitQuantityToggle = SitQuantityToggle;
exports["default"] = SitQuantityToggle;
//# sourceMappingURL=sit-quantity-toggle.cjs.js.map
