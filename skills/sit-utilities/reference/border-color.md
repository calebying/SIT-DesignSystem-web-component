# Canvas Border Color Utilities Skill
...
All Canvas border color utilities use the `sit:border-{semantic}-{modifier}` pattern with the `sit:` prefix.

> **Common mistake**: The word "color" never appears in the class name. Do NOT write `sit:border-color-default`, `sit:border-color-primary`, etc. The correct classes are `sit:border-default`, `sit:border-primary-default`, etc.
...
- `sit:border-{modifier}` — Foundational borders (no semantic color)
- `sit:border-{color}-{modifier}` — Semantic color borders

**Examples:**
- `sit:border-default` — Standard neutral border
- `sit:border-primary-default` — Brand-colored border, default emphasis
- `sit:border-danger-emphasis` — Error border, strong emphasis
...
- No special meaning, structural — `base` (use `sit:border-default`)
...
8. Always combine border color with a border-width class (`sit:border`) — color alone produces no visible border
