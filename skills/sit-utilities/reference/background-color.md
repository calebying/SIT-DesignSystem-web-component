# Canvas Background Color Utilities Skill

Helps developers use Canvas background color utilities that adapt to light/dark themes using semantic tokens.

## Core Concept

All Canvas background utilities use the `sit:bg-{variant}-{modifier}` pattern with the `sit:` prefix.
...
All tokens follow: `sit:bg-{semantic}-{level}-{emphasis}`

**Examples:**
- `sit:bg-surface-default` - Component, no semantic meaning, default emphasis
- `sit:bg-primary-surface-emphasis` - Component, primary brand color, strong emphasis
- `sit:bg-success-default` - Page level, success color, default emphasis
...
**For Components:**
- Default component bg → `sit:bg-surface-default`
- Need differentiation (badge on card) → `sit:bg-surface-raised`
- Want inverse look → `sit:bg-surface-inverse`
- Over unchanging content (image) → `sit:bg-surface-fixed-{light|dark}`
- Hover/active state → `sit:bg-translucent` or `sit:bg-subtle`
- Transparent but maintain box → `sit:bg-transparent`

**For Pages:**
- Default page bg → `sit:bg-default`
- Alternate section → `sit:bg-alternate`
- Full-screen overlay → `sit:bg-overlay`
- Fixed color section → `sit:bg-fixed-{light|dark}`
