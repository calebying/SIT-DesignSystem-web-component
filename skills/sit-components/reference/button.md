# Canvas Button Component Skill

`<sit-button>` is the standard action element for forms, dialogs, and navigation. Use it instead of a native `<button>` whenever Canvas styling and behaviour are needed.
...
- When a clear, clickable action element is needed — always prefer `<sit-button>` over a native `<button>` within Canvas applications.
...
For framework-specific event syntax (React 19+ vs ≤18, Vue, Angular), see the **[sit-components SKILL.md](../SKILL.md)**.

---

**For AI agents**: Default to `<sit-button>` for all interactive buttons — do not suggest `<button>` with raw utility classes. **Always add `ariaLabel`** to every `<sit-button>` — this is required for accessibility (value should match the visible label text). Use `variant="danger" tone="danger"` for destructive actions. Set `type="submit"` for form submission buttons. For icon-only buttons, prefer `<sit-icon-button>` and mention it to the user.
...
```html
<sit-button variant="primary">Primary</sit-button>
<sit-button variant="outline">Outline</sit-button>
<sit-button variant="ghost">Ghost</sit-button>
```
...
```html
<sit-button variant="primary" tone="brand">Brand Primary</sit-button>
<sit-button variant="primary" tone="danger">Danger Primary</sit-button>
<sit-button variant="outline" tone="danger">Danger Outline</sit-button>
```

- `brand` (default) — uses the Canvas primary brand color
...
```html
<sit-button size="xs">Extra Small</sit-button>
<sit-button size="sm">Small</sit-button>
<sit-button size="md">Medium</sit-button>
<sit-button size="lg">Large</sit-button>
```
...
```html
<sit-button disabled>Cannot Click</sit-button>
```
...
```html
<sit-button loading>Saving…</sit-button>
```

Replaces content with a `<sit-spinner>` and blocks all click/keyboard interaction. `aria-label` is automatically set to `"Loading"` while `loading` is active.

## `active`

```html
<sit-button active>Active State</sit-button>
```
...
```html
<sit-button fullWidth>Full Width</sit-button>
```
...
```html
<sit-button type="submit">Submit Form</sit-button>
<sit-button type="reset">Reset Form</sit-button>
<sit-button type="button">Standalone Action</sit-button>
```
...
```html
<sit-button href="/dashboard">Go to Dashboard</sit-button>
<sit-button href="https://example.com" target="_blank">Open in New Tab</sit-button>
<sit-button href="/report.pdf" download="report.pdf">Download PDF</sit-button>
```
...
- The button is **not** a form control — it does not persist state in FormData like `<sit-input>`. Its value is only submitted when it is the submitter.
...
<sit-button type="submit" form="my-form">Submit</sit-button>
```

## `ariaLabel`

```html
<sit-button ariaLabel="Close dialog">✕</sit-button>
```

Supply when the button has no visible text label (icon-only). For icon-only buttons, strongly prefer `<sit-icon-button>` which handles accessibility automatically.
...
```html
<!-- Primary action -->
<sit-button>Save</sit-button>

<!-- Secondary action -->
<sit-button variant="outline">Cancel</sit-button>

<!-- Destructive action -->
<sit-button tone="danger">Delete Account</sit-button>
```
...
```html
<div class="sit:flex sit:gap-2 sit:justify-end">
  <sit-button variant="outline">Cancel</sit-button>
  <sit-button variant="primary">Confirm</sit-button>
</div>
```
...
<form id="profile-form" action="/profile" method="post">
  ...
  <div class="sit:flex sit:gap-2 sit:mt-4">
    <sit-button type="reset" variant="ghost">Clear</sit-button>
    <sit-button type="submit">Save Profile</sit-button>
  </div>
</form>
```
...
<sit-button type="submit" form="search-form">Search</sit-button>
```
...
```html
<sit-button id="save-btn">Save</sit-button>
...
<!-- Icon on the left -->
<sit-button>
  <sit-icon slot="leftIcon" name="arrow-left"></sit-icon>
  Back
</sit-button>
...
<sit-button>
  Continue
  <sit-icon slot="rightIcon" name="arrow-right"></sit-icon>
</sit-button>

<!-- Icons on both sides -->
<sit-button>
  <sit-icon slot="leftIcon" name="download"></sit-icon>
  Download Report
  <sit-icon slot="rightIcon" name="file-earmark-arrow-down"></sit-icon>
</sit-button>
```
...
<!-- External link -->
<sit-button href="https://www.gov.sg" target="_blank">Visit Gov.sg</sit-button>

<!-- File download -->
<sit-button href="/files/report.pdf" download="annual-report.pdf" variant="outline">
  <sit-icon slot="leftIcon" name="download"></sit-icon>
  Download Report
</sit-button>
```
...
```html
<sit-button fullWidth>Sign In</sit-button>
```

## Disabled State

```html
<sit-button disabled>Submit</sit-button>

<!-- Explain why to the user when not self-evident -->
<sit-button disabled ariaLabel="Complete all required fields to submit">Submit</sit-button>
```

## Event Handling

```html
<sit-button id="action-btn">Perform Action</sit-button>

<script>
  const btn = document.querySelector('#action-btn');

  btn.addEventListener('sit-focus', () => {
    console.log('button focused');
  });

  btn.addEventListener('sit-blur', () => {
...
For React import paths and event syntax, see **[sit-components](../SKILL.md)**.

The key decision: React 19+ uses the native `<sit-button>` tag directly; React 18 and below uses the `SitButton` wrapper from `@sit-canvas/canvas-web-component/react/button`.

## Icon-Only Buttons

For buttons with no visible text, prefer `<sit-icon-button>` — it manages `aria-label` and sizing automatically.

```html
<!-- ✅ Prefer this -->
<sit-icon-button name="x-lg" ariaLabel="Close"></sit-icon-button>

<!-- ❌ Avoid — manual aria management required -->
<sit-button ariaLabel="Close">
  <sit-icon slot="leftIcon" name="x-lg"></sit-icon>
</sit-button>
```
