# Canvas Footer Component Skill

`<sit-footer>` is the standard footer for Singapore Government digital services. It renders a structured footer with mandatory links (Contact, Feedback, Privacy, Terms of Use) and supports optional site columns via `<sit-footer-item>`.
...
- When the footer layout needs to deviate significantly from the Canvas standard — use the *(default)* slot for full control, but remain aligned to government branding guidelines.
...
- `<sit-footer-item slot="items">` renders a column with a heading (`title` slot) and `<sit-link>` children.
...
**`items` slot** — one `<sit-footer-item slot="items">` per column. Inside each `<sit-footer-item>`:
- `title` slot: the column heading (e.g. `<div slot="title">Guides</div>`)
- Default slot: `<sit-link><a href="...">Label</a></sit-link>` elements for column navigation links. Do not use bare `<a>` tags.

**Default slot (`<sit-footer>`)** — full custom column layout that replaces the `items` slot entirely. Use only when the standard grid layout is insufficient; do not mix with `items` slot in the same footer.
...
- **Column link format**: each `<sit-footer-item>` column uses `<sit-link><a href="...">Label</a></sit-link>` — do not use bare `<a>` tags directly.
- **`layout` prop for sidebar apps**: set `layout="sidebar"` when the footer sits alongside a collapsible or persistent `<sit-sidebar>`. This adjusts `max-width` to match `.sit-container-sidebar`. Overlay sidebars do not affect content width, so keep `layout="default"`.
...
- **Empty `<sit-footer-item>`**: renders a column with a heading but no links — always include at least one `<sit-link>` inside each `<sit-footer-item>`.
...
**Minimal footer (just the legal links bar)?** → Use `<sit-footer>` with no slots

**Add site column links?** → Use `<sit-footer-item slot="items">` for each column
...
>
  ...
  <sit-footer-item slot="items">
    ...
    <sit-link><a href="/guides/getting-started">Getting Started</a></sit-link>
    <sit-link><a href="/guides/installation">Installation</a></sit-link>
    <sit-link><a href="/guides/migration">Migration Guide</a></sit-link>
  </sit-footer-item>

  <sit-footer-item slot="items">
    <div slot="title">Resources</div>
    <sit-link><a href="/resources/forms">Forms</a></sit-link>
    <sit-link><a href="/resources/templates" target="_blank">Templates</a></sit-link>
  </sit-footer-item>
</sit-footer>
```
...
| `items` | `<sit-footer-item>` column elements (auto-formatted in a grid) |
...
| *(default)* | `<sit-link>` elements for the column links |
...
2. Use `<sit-footer-item slot="items">` for each column; place `<sit-link>` elements inside for navigation links.
...
6. When generating a sidebar app layout (collapsible or persistent `<sit-sidebar>`), always add `layout="sidebar"` and `tone="neutral"` to `<sit-footer>`. For overlay sidebars or layouts without a sidebar, omit the attribute or use `layout="default"`.
