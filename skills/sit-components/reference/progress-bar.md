# Canvas Progress Bar Component Skill

`<sit-progress-bar>` renders a horizontal progress bar. Always provide `ariamin`, `ariamax`, and `arialabel` for accessibility.
...
- When the duration is unknown and indeterminate — use `<sit-spinner>` instead.
- For step navigation in a multi-step wizard — use `<sit-stepper>` instead.
