# Canvas Combo Box Component Skill

`<sit-combo-box>` is a searchable select with client-side filtering out of the box. Add `multiSelect` to allow multiple selections (displayed as dismissible badges). Enable `async` for server-side filtering.
...
- For small, fixed lists (fewer than ~10 options) → use `<sit-select>` or `<sit-radio>` instead.
- For binary or simple choices → use `<sit-checkbox>` or `<sit-radio>`.
- When input must be fully freeform with no option list → use `<sit-input>`.
...
- Set `async` to disable client-side filtering and handle results server-side via the `sit-input` event.
...
- **Async data**: set `async`, listen to `sit-input` for `event.detail.displayValue`, fetch results, then replace `<sit-combo-box-option>` children. Set `loading` to indicate fetch in progress.
...
- **Infinite scroll / lazy load**: set `scrollBottomOffset` (number, pixels) to fire `sit-scroll-end` before the user reaches the very bottom of the menu. Listen to `sit-scroll-end` to load and append more `<sit-combo-box-option>` elements. A positive offset (e.g. `20`) gives time to fetch data before the user hits the end.
...
- **Clearing selection**: `clearable` resets `element.value` to empty — ensure downstream listeners on `sit-change` handle an empty value gracefully.
...
For detailed form pattern guidance (when to pair fields, spacing, responsive behavior, multi-select rules), see the [sit-blocks form layout skill](../sit-blocks/reference/form.md).

## Quick Decision Guide

**Single-select searchable dropdown?** → Default `<sit-combo-box>`

**Multi-select?** → Add `multiSelect`

**Custom filter function?** → Set `filterFunction` via JS property

**Server-side async filtering?** → Add `async`, then update the option list in the `sit-input` event handler

**Show a clear button?** → Add `clearable`

**Full-width selected badges?** → Add `badgeFullWidth`

**Infinite scroll / lazy-load more options?** → Set `scrollBottomOffset` and listen to `sit-scroll-end`
...
>
  <sit-combo-box-option value="sg">Singapore</sit-combo-box-option>
  <sit-combo-box-option value="my">Malaysia</sit-combo-box-option>
  <sit-combo-box-option value="au">Australia</sit-combo-box-option>
  <sit-combo-box-option value="uk" disabled>United Kingdom (unavailable)</sit-combo-box-option>
</sit-combo-box>
...
>
  ...
  <sit-combo-box-option value="engineering">Engineering</sit-combo-box-option>
  <sit-combo-box-option value="product">Product</sit-combo-box-option>
</sit-combo-box>
...
<script>
  ...
  combo.addEventListener("sit-input", async e => {
    ...
    combo.innerHTML = results.map(u =>
      `<sit-combo-box-option value="${u.id}">${u.name}</sit-combo-box-option>`
    ).join("");
    combo.loading = false;
  });
  combo.addEventListener("sit-select", () => {
...
| `noValidate` | boolean | `false` | Disables native and Canvas validation on this component |
...
| `scrollBottomOffset` | number | `0` | Pixels before the bottom of the menu at which `sit-scroll-end` fires — must be non-negative; negative values are treated as `0` |
...
|---|---|---|
| `<sit-combo-box>` | *(default)* | `<sit-combo-box-option>` elements |
| `<sit-combo-box>` | `invalidIcon` | Custom icon shown when the field is invalid |
| `<sit-combo-box-option>` | *(default)* | Option label text |

## Methods

| Method | Signature | Purpose |
|---|---|---|
| `setInvalid` | `(bool: boolean) => void` | Programmatically set or clear invalid state; emits `sit-invalid` or `sit-valid` |
...
## Events (`<sit-combo-box>`)

| Event | Detail | When |
|---|---|---|
| `sit-select` | — | An option is selected (read `element.value`) |
| `sit-change` | — | The combo box value changes |
| `sit-input` | `{ displayValue: string }` | User types into the input field |
| `sit-focus` | — | Input gains focus |
| `sit-blur` | — | Input loses focus |
| `sit-scroll-end` | — | Menu scroll position reaches the bottom (within `scrollBottomOffset` pixels) — use to trigger lazy loading |
| `sit-invalid` | — | Emitted when `setInvalid(true)` is called |
| `sit-valid` | — | Emitted when `setInvalid(false)` is called |
| `sit-show` | — | Dropdown menu is about to show |
| `sit-after-show` | — | Dropdown menu has finished showing |
| `sit-hide` | — | Dropdown menu is about to hide |
| `sit-after-hide` | — | Dropdown menu has finished hiding |

---

**For AI agents**:
1. For single-select, `element.value` is the selected option's `value`. For multi-select, it is a semicolon-separated string (`;`).
2. For async filtering, set `async` (boolean attribute), listen to `sit-input` for `event.detail.displayValue`, fetch results, then populate `<sit-combo-box-option>` children.
...
5. `<sit-combo-box>` supports `hasFeedback` and `invalidFeedback` like other Canvas form components. Set `hasFeedback` to enable validation styling and `invalidFeedback` to display a custom error message.
6. **Infinite scroll**: set `scrollBottomOffset` (number of pixels) and listen to `sit-scroll-end` to append more `<sit-combo-box-option>` elements before the user reaches the end of the list. The event fires once per scroll-to-bottom gesture and resets when the user scrolls back up.
