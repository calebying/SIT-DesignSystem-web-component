# Canvas Icon Component Skill

`<sit-icon>` renders SVG icons from the Canvas built-in registry. Use it instead of `<img>` tags or external icon libraries when building with Canvas components.
...
- To add an icon from the Canvas registry alongside text, inside buttons, links, or as standalone decorative elements.
- Inside `<sit-button>`, `<sit-link>`, `<sit-icon-list>`, and other Canvas components that have dedicated icon slots.
- For feature or status icons in cards, alerts, badges, and banners.

### When NOT to use

- For icons not in the Canvas registry — source them via a custom SVG or an alternative icon library instead.
...
- Inside `<sit-link>` with a manually set `size` — icon sizing is managed automatically by the link component; do not override it.
- As interactive elements — `<sit-icon>` is non-interactive; use `<sit-icon-button>` for clickable icons.
...
- When used inside `<sit-link>`, icon size is managed automatically by the parent — do not set `size` manually.
...
- **Inside `<sit-link>`**: `<sit-link>` sets icon size based on its own `size` attribute — adding a `size` on the icon will be overridden; omit it entirely.
- **Inside `<sit-icon-list>`**: icon size must be set explicitly to match the list `size` (see icon-list skill) — this is the exception where manual sizing is required.
...
- **`size` inside `<sit-link>`**: is overridden by the parent link — omit to avoid confusion.
- **`size` inside `<sit-icon-list>`**: must be set manually (the list does not auto-propagate size to icons) — always align to the list size mapping.
...
> When used inside `<sit-link>`, icon sizing is managed automatically — do not set `size` manually.

```html
<!-- Standalone informative icon (add ariaLabel) -->
<sit-icon name="announcement" ariaLabel="Announcement"></sit-icon>

<!-- Specific size -->
<sit-icon name="arrow-right" size="sm" ariaLabel="Arrow right"></sit-icon>

<!-- Large standalone icon -->
<sit-icon name="star" size="xl" ariaLabel="Star"></sit-icon>
...
2. **Only use names from the list above.** Before using any icon name, verify it exists in the registry. If the name the user asked for is not in the list, tell them explicitly — do not silently substitute. Instead, suggest the closest available alternative from the list and explain why. Example: *"There is no `funnel` icon in the Canvas registry. The closest available option is `bi-funnel`."*
...
4. Icons render from the internal Canvas SVG registry — they are not `<img>` tags and do not accept `src`.
5. Default size is `lg` — if a user needs a smaller inline icon, explicitly set `size="sm"` or `size="md"`.
6. Inside `<sit-link>`, `<sit-badge>`, and similar components, icon size is managed automatically; do not override it with `size`.
7. **Accessibility**: by default, the SVG is decorative (`aria-hidden="true"`). When the icon IS the content (standalone, no surrounding text label), add `ariaLabel` to make it informative. When used inside a labelled parent (e.g. `<sit-button>`, `<sit-icon-button>`), omit `ariaLabel` — the parent provides the accessible name.
