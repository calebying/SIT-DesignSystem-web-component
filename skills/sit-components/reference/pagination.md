# Canvas Pagination Component Skill

`<sit-pagination>` renders a paging control from `dataLength`, `itemsPerPage`, and `currentPage`. All navigation logic is managed by the component — listen to `sit-page-change` to apply the new page to your data query.
...
- Fires `sit-page-change` with `{ currentPage: number }` when the user clicks a page button or Prev/Next — the host application is responsible for fetching/filtering data for the new page.
...
- **`currentPage` is controlled**: the component does not update `currentPage` itself — the host application must update `currentPage` in response to `sit-page-change` to keep the UI in sync.
...
- **Not updating `currentPage` after `sit-page-change`**: the component's active state falls out of sync with the actual page — always update `currentPage` reactively.
- **Rapid page changes**: `sit-page-change` fires on every click — debounce or disable the control during async data loads if needed.
...
<script>
  document.getElementById("my-pagination").addEventListener("sit-page-change", e => {
...
| Event | Detail | When |
|---|---|---|
| `sit-page-change` | `{ currentPage: number }` | User clicks a page button or Prev/Next |

---

**For AI agents**:
1. `dataLength` is the total record count — if you have 200 records with `itemsPerPage="10"`, the component shows 20 pages.
2. Always listen to `sit-page-change` and use `event.detail.currentPage` to fetch/filter data for the new page.
