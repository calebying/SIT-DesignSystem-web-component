# Canvas Table of Contents Component Skill

`<sit-table-of-contents>` renders a list of anchor links for in-page navigation. Provide a heading via the default slot and a list of `<li>` items containing `<sit-link>` elements via the `contents` slot.
...
- For site-level or multi-page navigation — use `<sit-mainnav>` or `<sit-sidenav>` instead.
...
- The `contents` slot accepts `<li>` elements, each containing a `<sit-link>` wrapping an `<a href="#section-id">`.
...
- **No active state tracking**: `<sit-table-of-contents>` does not highlight the currently visible section as the user scrolls — implement scroll-spy behaviour manually if needed.
...
**Standard table of contents for a content page?** → Use `<sit-table-of-contents>` with a heading in the default slot and `<li>` items in the `contents` slot

**Each entry links to a page section?** → Use `<sit-link>` with `<a href="#section-id">` inside each `<li>`
...
<sit-table-of-contents>
  <h5>Contents</h5>
  <li slot="contents">
    <sit-link><a href="#overview">Overview</a></sit-link>
  </li>
  <li slot="contents">
    <sit-link><a href="#installation">Installation</a></sit-link>
  </li>
  <li slot="contents">
    <sit-link><a href="#usage">Usage</a></sit-link>
  </li>
  <li slot="contents">
    <sit-link><a href="#api">API Reference</a></sit-link>
  </li>
  <li slot="contents">
    <sit-link><a href="#changelog">Changelog</a></sit-link>
  </li>
</sit-table-of-contents>
...
| `contents` | `<li>` elements, each containing a `<sit-link>` with an anchor `<a href="#">` |
...
2. Each `contents` slot item is a `<li>` element containing `<sit-link><a href="#section-id">Label</a></sit-link>`.
