# Canvas Drawer Component Skill

`<sit-drawer>` is a panel that slides in from a screen edge and overlays the page content. Use it for secondary navigation, filters, or any contextual content that shouldn't interrupt the main layout.
...
- For critical, blocking decisions that require immediate user action before continuing — use `<sit-modal>` instead.
- For simple confirmations or short messages — use `<sit-modal>` or `<sit-alert>`.
- As a replacement for a page — if the content warrants its own URL and navigation, use a separate route.
- When the interaction is passive and non-interruptive — use `<sit-alert>` or `<sit-toast>` instead.
...
- Renders a backdrop overlay; clicking the overlay fires `sit-request-close` with `source: "overlay"`.
- Pressing `Escape` fires `sit-request-close` with `source: "keyboard"`.
- The built-in close button fires `sit-request-close` with `source: "close-button"`.
- Call `event.preventDefault()` on `sit-request-close` to block closing — e.g. when the user has unsaved changes.
...
- `sit-initial-focus` fires when the drawer opens — call `event.preventDefault()` to manually control which element receives focus.
...
**`footer` slot** — action buttons: a primary save/apply action (`<sit-button variant="primary">`) and a cancel action (`<sit-button variant="link">`). Both must call `hide()` explicitly.
...
- **`sit-request-close` guard**: inspect `event.detail.source` to apply different close guards per trigger — e.g. allow the close button but block overlay clicks when unsaved changes exist.
...
- **Focus management**: by default, focus moves to the first focusable element inside the drawer on open. Cancel `sit-initial-focus` and set focus manually when the default target is not the right choice.
...
- **`sit-after-show` / `sit-after-hide`**: use these for post-animation actions (e.g. loading content after open, cleanup after close) rather than `sit-show` / `sit-hide` which fire before animation completes.

## Edge Cases

- **Unsaved changes guard**: if `sit-request-close` is always prevented, the user has no way to close the drawer — always provide an explicit action button that calls `hide()`.
...
<sit-button id="open-btn">Open Drawer</sit-button>
<sit-drawer id="my-drawer" size="sm" placement="end">
  <h4 slot="title">Drawer Title</h4>
  <p slot="description">Brief drawer description</p>
  Main content goes in the default slot.
  <div slot="footer">
    <sit-button id="close-btn">Close</sit-button>
  </div>
</sit-drawer>
...
<sit-drawer open placement="start">
  <h4 slot="title">Left Drawer</h4>
  Content here.
</sit-drawer>

<!-- Prevent closing when user clicks overlay or presses Escape -->
<sit-drawer id="confirm-drawer">
  <h4 slot="title">Unsaved Changes</h4>
  You have unsaved changes.
  <div slot="footer">
    <sit-button id="discard">Discard</sit-button>
    <sit-button variant="primary" id="save">Save</sit-button>
  </div>
</sit-drawer>

<script>
  document.getElementById("confirm-drawer").addEventListener("sit-request-close", e => {
...
|---|---|---|---|
| `sit-show` | No | — | Drawer begins opening |
| `sit-after-show` | No | — | Drawer fully open (animation complete) |
| `sit-hide` | No | — | Drawer begins closing |
| `sit-after-hide` | No | — | Drawer fully closed (animation complete) |
| `sit-initial-focus` | Yes | — | Drawer is open and ready for focus; call `event.preventDefault()` to manage focus manually |
| `sit-request-close` | Yes | `{ source: 'close-button' \| 'keyboard' \| 'overlay' }` | User attempts to close — call `event.preventDefault()` to keep it open |
...
3. `sit-request-close` is cancelable — call `event.preventDefault()` to prevent closing (e.g. to confirm unsaved changes). Check `event.detail.source` to distinguish user actions.
4. `contained` requires the parent element to have `position: relative` and explicit height for proper scoping.
5. `sit-initial-focus` is cancelable — call `event.preventDefault()` to control which element receives focus when the drawer opens.
6. Footer slot typically holds `<sit-button>` elements for primary and secondary actions.
