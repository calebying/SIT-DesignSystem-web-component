# Canvas Tooltip Component Skill

`<sit-tooltip>` wraps a target element and shows a small text popup on hover, focus, or click. The tooltip content must be plain text only — no HTML.
...
- For icon-only buttons where `<sit-icon-button>` does not have a visible text label.
...
<sit-tooltip content="More information about this field" placement="bottom">
  <sit-icon name="info-circle" ariaLabel="Information" tabindex="0"></sit-icon>
</sit-tooltip>

<!-- Tooltip triggered only on click -->
<sit-tooltip content="Copied to clipboard!" trigger="click" placement="top">
  <sit-button ariaLabel="Copy">Copy</sit-button>
</sit-tooltip>

<!-- Tooltip with focus trigger only (keyboard accessible) -->
<sit-tooltip content="Required field" trigger="focus">
  <input type="text" placeholder="Enter your name" />
</sit-tooltip>

<!-- Tooltip on a non-focusable SVG element -->
<sit-tooltip content="Quick reference guide" placement="right">
...
|---|---|---|
| `sit-show` | No | Tooltip begins showing |
| `sit-after-show` | No | Tooltip fully visible (animation complete) |
| `sit-hide` | No | Tooltip begins hiding |
| `sit-after-hide` | No | Tooltip fully hidden (animation complete) |
