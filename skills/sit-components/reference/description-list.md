# Canvas Description List Component Skill

`<sit-description-list>` renders a single label–value row. `<sit-description-list-group>` wraps multiple rows with an optional group title and description. Use this instead of raw `<dl>/<dt>/<dd>` elements for consistent Canvas styling.
...
- When multiple rows share a logical group — wrap them in `<sit-description-list-group>` with a title.

### When NOT to use

- For editable form fields — use `<sit-input>`, `<sit-select>`, or other form components.
- For tabular data with multiple columns — use `<sit-table>` instead.
- When no label is needed — use plain text or `<sit-badge>` for single values.
- For navigation or interactive lists — use `<sit-icon-list>` or a nav component.

## Behaviour

- `<sit-description-list>` renders one label (default slot) and one value (`data` slot) per row.
- `<sit-description-list-group>` wraps multiple rows and accepts an optional `title` and `description` slot.
...
- **Stacking propagation**: `stacked` on the group sets an internal property on each child `<sit-description-list>` — setting it manually on individual rows has no visual effect beyond what the group already sets.
...
- **Rich value content**: the `data` slot accepts any inline HTML — `<sit-badge>`, `<a>`, `<strong>`, and spans are all valid; avoid block-level elements.
...
- **Single row without group**: `<sit-description-list>` works standalone; no `<sit-description-list-group>` wrapper is required for a single row.
...
- One label–value pair → `<sit-description-list>` alone
- Multiple rows with a shared title → wrap in `<sit-description-list-group>`
...
<!-- Stacked layout -->
<sit-description-list-group stacked>
  <sit-description-list>
    Label
    <span slot="data">Value</span>
  </sit-description-list>
...
## API Summary — `<sit-description-list-group>`

| Attribute | Type | Default | Purpose |
|---|---|---|---|
| `stacked` | boolean | `false` | Stacks label above value; propagated to all child `sit-description-list` elements |
| `bordered` | boolean | `false` | Adds a border; propagated to all child `sit-description-list` elements |

## API Summary — `<sit-description-list>`
...
## Slots — `<sit-description-list-group>`

| Slot | Purpose |
|---|---|
| *(default)* | `<sit-description-list>` elements |
| `title` | Optional group title |
| `description` | Optional group description |

## Slots — `<sit-description-list>`
...
**For AI agents**:
1. Always use `<sit-description-list-group>` + `<sit-description-list>` for key-value data — never suggest raw `<dl>/<dt>/<dd>`.
2. Set `stacked` and `bordered` on the **group**, not on individual `<sit-description-list>` elements — the group propagates them automatically.
3. The `data` slot on `<sit-description-list>` holds the value; the default slot holds the label.
