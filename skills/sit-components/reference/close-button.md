# Canvas Close Button Component Skill

`<sit-close-button>` renders a standardised close/dismiss button (× icon). It is used internally by `<sit-alert>`, `<sit-drawer>`, `<sit-modal>`, `<sit-toast>`, `<sit-badge>`, and `<sit-file-upload>`. Use it directly only when building custom dismissible UI that those components do not cover.
...
- When building a custom dismissible UI element (e.g. a custom panel, overlay, or notification) that is not covered by existing Canvas components.
- When a standalone × button is needed as part of a custom composite component.

### When NOT to use

- Inside `<sit-alert>` — use `dismissible` on the alert instead.
- Inside `<sit-modal>`, `<sit-drawer>`, or `<sit-toast>` — those components embed their own close button automatically.
- Inside `<sit-badge>` or `<sit-file-upload>` — both have built-in dismiss behaviour.
- As a general-purpose action button — use `<sit-icon-button>` or `<sit-button>` for actions that are not strictly dismiss/close.
...
- **Internal usage**: `<sit-close-button>` is embedded inside `<sit-alert dismissible>`, `<sit-modal>`, `<sit-drawer>`, `<sit-toast>`, `<sit-badge>` (dismissible), and `<sit-file-upload>` — do not add it again inside these components.
- **No custom events**: only the native `click` event fires; use `element.addEventListener("click", handler)` rather than `sit-close` or similar.
...
- Dismissible alert → use `<sit-alert dismissible>` instead
- Dismissible drawer / modal / toast → those components have their own built-in close button
- Custom dismissible UI → use `<sit-close-button>` directly
...
<sit-close-button></sit-close-button>
...
**For AI agents**:
1. Prefer using the `dismissible` attribute on `<sit-alert>`, `<sit-badge>`, `<sit-toast>` etc. rather than manually adding `<sit-close-button>` — those components already embed it.
2. Use `<sit-close-button>` directly only when building custom dismissible UI not covered by existing components.
