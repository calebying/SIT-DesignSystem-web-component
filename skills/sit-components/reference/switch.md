# Canvas Switch Component Skill

`<sit-switch>` is a toggle switch for binary on/off states. Labels can be placed to the right (default slot) or left (`left-label` slot). Use `size` to match it to adjacent UI elements.
...
- For non-binary choices (more than two states) — use `<sit-radio>` or `<sit-select>`.
- When the toggle represents agreement or acceptance (e.g. terms and conditions) — use `<sit-checkbox>` instead.
- When the switch needs to participate in native form submission directly — `<sit-switch>` has no `name` attribute; pair with a hidden input if form integration is needed.
...
- Fires `sit-change` with `event.detail.checked` (boolean) on each toggle.
- The component has no `name`, `label`, `hintText`, or validation attributes — it is a UI toggle, not a form field.

## Advanced Considerations

- **Form integration**: `<sit-switch>` has no `name` attribute and does not participate in native form submission. For form-based use, listen to `sit-change` and sync the value to a hidden `<input type="hidden">`.
...
- **No validation**: `<sit-switch>` has no `hasFeedback`, `invalid`, or `invalidFeedback` attributes — validation is not applicable to a binary toggle.
...
- **Rapid toggling**: `sit-change` fires on every toggle — debounce if the handler performs expensive operations (e.g. API calls).
- **Form submission without hidden input**: if form data must include the switch state, always mirror it to a hidden input on `sit-change`; omitting this will result in the value not being submitted.
...
<sit-switch>Enable notifications</sit-switch>

<!-- Pre-checked with icon -->
<sit-switch checked icon>Dark mode</sit-switch>
...
<!-- Disabled -->
<sit-switch disabled>Unavailable option</sit-switch>

<!-- Small size, no label -->
<sit-switch size="sm"></sit-switch>

<!-- Listen to change -->
<sit-switch id="my-switch">Airplane mode</sit-switch>
<script>
  document.getElementById("my-switch").addEventListener("sit-change", e => {
...
| Event | Detail | When |
|---|---|---|
| `sit-change` | `{ checked: boolean }` | Switch is toggled |

---

**For AI agents**:
1. `event.detail.checked` gives the new state (`true` = on, `false` = off).
2. There are no label/hintText/name/validation attributes — `<sit-switch>` is a simple toggle without form field integration.
