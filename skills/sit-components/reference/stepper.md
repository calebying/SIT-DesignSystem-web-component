# Canvas Stepper Component Skill

`<sit-stepper>` renders a step indicator and manages a multi-step workflow. Steps can be defined in two ways:

1. **Recommended**: Using `<sit-step>` child components (preferred approach)
2. **Deprecated**: Using the `steps` property with `IStepMetaData` objects (legacy, maintained for backwards compatibility)

Navigation is controlled programmatically via public methods or the `sit-arrived` event.
...
- Steps are declared as `<sit-step>` child elements of `<sit-stepper>`.
- Each `<sit-step>` has a `stepHeader` attribute (required) and optional `iconName`, `disabled`, and `completed` properties.
...
- If `<sit-step>` children are provided, the `steps` property will be ignored.
...
- Each method fires a corresponding event: `sit-next-step`, `sit-previous-step`, `sit-last-step`, `sit-first-step`, `sit-reset`.
- `sit-arrived` fires after any navigation method completes — read `activeStep` in the handler to determine the new step.
...
- **Framework integration**: In `sit-step` components, the `component` property is framework-agnostic — pass a Vue/React/Angular component reference, or a plain string identifier in vanilla JS.
...
- **`data-clickable` on slotted elements**: when a `<sit-step>` contains interactive elements in its default slot (e.g. `<a>` or `<button>`), add the `data-clickable` attribute to those elements so their clicks are handled independently and do **not** bubble up to trigger step navigation.
- **`firstStep()` / `lastStep()`**: useful for "Back to start" or "Skip to review" actions outside the standard Prev/Next flow.
- **`iconName` per step**: optional; sourced from the Canvas icon set. Omit to show the default step number. Consistent use across all steps is recommended.
...
- **`hasDefaultSlot` property**: internal property that tracks whether `<sit-step>` children are slotted. Used to determine whether to render fallback steps from the `steps` property.
...
- **`reset()` mid-flow**: calling `reset()` clears step statuses and returns to step 0 — ensure the content renderer also resets to the initial state via the `sit-reset` event.
- **`clickable` with validation**: if steps require prior completion before proceeding, disable `clickable` or implement a guard in the `sit-arrived` handler to redirect invalid jumps.
...
- **Mixing sit-step children with steps property**: if `<sit-step>` children are provided, the `steps` property is ignored entirely.

## Quick Decision Guide

**Use sit-step children (recommended)?** → `<sit-stepper><sit-step stepHeader="..."></sit-step></sit-stepper>`
...
**Track current step?** → Listen to `sit-arrived` or check `activeStep`

## Code Examples

### Using sit-step children (Recommended)

```html
<!-- HTML example with sit-step children -->
<sit-stepper id="stepper" activeStep="0">
  <sit-step stepHeader="Personal Info" iconName="person-fill"></sit-step>
  <sit-step stepHeader="Address" iconName="house-fill"></sit-step>
  <sit-step stepHeader="Review"></sit-step>
  <sit-step stepHeader="Confirm"></sit-step>
</sit-stepper>
<div id="step-content"></div>
<sit-button id="prev-btn" disabled>Previous</sit-button>
<sit-button id="next-btn">Next</sit-button>
...
<script>
  ...
  stepper.addEventListener("sit-arrived", e => {
...
|---|---|---|---|
| `steps` | `IStepMetaData[]` | `[]` | **(Deprecated)** Array of step definitions (set via JS property). Ignored if `<sit-step>` children are provided. |
...
| `hasDefaultSlot` | boolean | `false` | **(Internal)** Indicates whether `<sit-step>` children are slotted |
...
| `iconName` | string | undefined | Custom icon name for the step indicator (from Canvas icon set) |
...
Add `data-clickable` to any element placed in a `<sit-step>`'s default slot to prevent its click from bubbling up and triggering step navigation. Applies to any tag, but most commonly used on `<a>` and `<button>` elements.

```html
<sit-step stepHeader="Details">
...
| `iconName` | string | No | Custom icon name for the step indicator (from Canvas icon set) |
...
|---|---|---|
| `sit-next-step` | No | `nextStep()` was called |
| `sit-previous-step` | No | `previousStep()` was called |
| `sit-last-step` | No | `lastStep()` was called |
| `sit-first-step` | No | `firstStep()` was called |
| `sit-arrived` | No | The stepper has arrived at a new step; use `activeStep` to get the current index |
| `sit-reset` | No | `reset()` was called |

---

**For AI agents**:
1. **Prefer `<sit-step>` children** when possible — this is the recommended approach.
...
3. In `sit-step` components, the `component` property is framework-agnostic: in Vue/React/Angular it can be a component reference; in vanilla JS it can be an identifier string for your step content renderer.
4. Listen to `sit-arrived` to update the displayed step content — read `activeStep` to know which step is now visible.
5. `activeStep` is 0-indexed: step 0 is the first step.
6. Use `sit-button` elements outside the stepper for Prev/Next controls; call `.previousStep()` and `.nextStep()` from button click handlers.
7. `iconName` is optional per step — defaults to a step number.
8. If `<sit-step>` children are provided, the `steps` property is completely ignored — do not mix both approaches.
9. If a `<sit-step>` slot contains interactive elements (`<a>`, `<button>`, `<sit-button>`) whose clicks should not navigate the stepper, add the `data-clickable` attribute to those elements.
