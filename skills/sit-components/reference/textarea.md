# Canvas Textarea Component Skill

`<sit-textarea>` is a multi-line text input with built-in label, hint text, character limit feedback, resize control, spellcheck, and validation support.
...
### When NOT to use
- When input is expected to be short or single-line — use `<sit-input>` instead
- When character count is extremely limited — use `<sit-input>` with validation
...
- `noValidate` is **not available** on `<sit-textarea>` — unlike `<sit-input>`, browser constraint validation cannot be disabled via an attribute. Use `invalid` attribute + `setInvalid()` method for server-driven error flows.
...
| `hasFeedback` | string | — | Controls validation UI; like `<sit-input>`, supports `"style"`, `"text"`, `"both"` |
...
| Event | When |
|---|---|
| `sit-change` | Committed value change |
| `sit-input` | Each keystroke that changes the value |
| `sit-focus` | Textarea gains focus |
| `sit-blur` | Textarea loses focus |
...
4. `setInvalid('message')` programmatically marks the textarea invalid with a custom message — use for server-driven or cross-field validation. Unlike `<sit-input>`, `noValidate` is not available on this component, so use `invalid` + `setInvalid()` to fully control error state.
