# Canvas Divider Component Skill

`<sit-divider>` renders a separator line to visually group or divide adjacent content. Use it instead of a native `<hr>` whenever Canvas consistent styling is needed.
...
- Do not use a raw `<hr>` or border utility — always use `<sit-divider>` for consistent Canvas styling.
...
**For AI agents**:
1. Always use `<sit-divider>` for separator lines — do not suggest raw `<hr>` elements or border utilities.
