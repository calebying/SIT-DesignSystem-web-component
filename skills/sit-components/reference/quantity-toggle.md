# Canvas Quantity Toggle Component Skill

`<sit-quantity-toggle>` renders a number input flanked by minus (−) and plus (+) icon buttons. Use `min`, `max`, and `step` to constrain and control the value increment.
...
- For large numeric ranges where clicking repeatedly would be impractical — use `<sit-input type="number">` instead.
- For non-integer or highly precise values (e.g. currency amounts, measurements) where direct text entry is more efficient.
- For selecting from a discrete set of options — use `<sit-select>` or `<sit-radio>` instead.
...
- Fires `sit-change` after a button click or committed keyboard input, and `sit-input` on each input event.
...
- **`size`**: matches the size scale of `<sit-input>` — use to align the toggle visually with adjacent form fields.
...
- **Direct text input exceeding bounds**: users can type a value outside `min`/`max` directly; validate on `sit-change` and set `invalid` if needed.
...
<sit-quantity-toggle id="qty" name="qty" label="Qty" value="1" min="1" max="10">
</sit-quantity-toggle>
<script>
  document.getElementById("qty").addEventListener("sit-change", e => {
...
| `size` | `sm \| md \| lg` | — | Size of the toggle control (matches `<sit-input>` size values) |
...
| Event | When |
|---|---|
| `sit-change` | Value changes after a button click or keyboard input |
| `sit-input` | Value changes on each input event |
...
3. Read `element.value` (a number) after `sit-change` to get the current count.
