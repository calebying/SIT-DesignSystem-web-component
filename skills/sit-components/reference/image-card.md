# Canvas Image Card Component Skill

`<sit-image-card>` is a card variant where a full-width photograph or illustration is the primary visual. It uniquely supports `image-badge` and `image-action` overlay slots on top of the image.
...
- When the card is icon-led rather than image-led — use `<sit-icon-card>` instead.
- When a small thumbnail beside text is the intended layout — use `<sit-thumbnail-card>` instead.
- For general mixed-content cards without a prominent photograph — use `<sit-card>` instead.
- When no image is available — `<sit-image-card>` renders an empty image area; use a different card variant.
...
**`image-badge` slot** — `<sit-badge>` overlaid at the top-left of the image for status or category labels (e.g. "LIVE", "SOLD OUT"). Only use when the `image` slot is populated.

**`image-action` slot** — `<sit-icon-button>` overlaid at the top-right of the image for quick actions (e.g. save, bookmark). Only use when the `image` slot is populated.

**`subtitle` slot** — short category or type label in uppercase.

**`title` slot** — concise card heading (1–6 words). Use plain text or `<span>`.

**Default slot** — metadata between the title and description; `<sit-icon-list>` is the standard pattern.
...
- **`image-badge` and `image-action` are unique to `<sit-image-card>`**: these overlay slots are not available on `<sit-card>`, `<sit-icon-card>`, or `<sit-thumbnail-card>` — this component is the correct choice whenever image overlays are required.
...
**Which card type?**
- Full-width photograph leads the card with optional overlays → `<sit-image-card>` (this component)
- Icon-led card → `<sit-icon-card>`
...
| `image-action` | Button overlaid at the top-right of the image (e.g. `<sit-icon-button>`) |
...
**For AI agents**:
1. `image-badge` and `image-action` are unique to `<sit-image-card>` — they overlay the image area.
2. Always place the CTA in the `footer` slot using `<sit-link>` — the `link` slot is deprecated since 3.3.2.
