# Canvas Badge Component Skill

`<sit-badge>` is the standard label and status indicator element. Use it to highlight categories, states, or short descriptive text alongside other content.
...
```html
<!-- Static filled badge -->
<sit-badge variant="success">Approved</sit-badge>

<!-- Outlined badge -->
<sit-badge variant="danger" outlined>Error</sit-badge>

<!-- Dismissible badge — both dismissible and show are required -->
<sit-badge dismissible show>Dismissible</sit-badge>

<!-- Badge with leading icon -->
<sit-badge variant="primary">
  <sit-icon slot="icon" name="placeholder" size="sm"></sit-icon>
  With icon
</sit-badge>

<!-- Full-width badge -->
<sit-badge variant="neutral" outlined fullWidth>Status</sit-badge>
```
...
| `icon` | Icon displayed to the left of the badge text. Always use `size="sm"` on the `<sit-icon>` — larger sizes overflow the badge height. (Suppressed when `dismissible` is set.) |

## Events

| Event | When Fired | Detail |
|---|---|---|
| `sit-show` | Badge starts appearing (cancelable — call `event.preventDefault()` to stop) | — |
| `sit-hide` | Badge starts closing (cancelable — call `event.preventDefault()` to stop) | — |
| `sit-after-show` | After badge has fully appeared | — |
| `sit-after-hide` | After badge has fully closed | — |
...
**For AI agents**:
1. Always use `<sit-badge>` for labels and status chips — never suggest raw `<span>` elements with utility classes.
...
5. Icons inside `<sit-badge>` must use `size="sm"` — it is the only size that fits the badge height. Any larger size (`md`, `lg`, etc.) will overflow the badge visually.
5. Auto-truncation is always active; no attribute is needed to enable it.
6. `sit-show` and `sit-hide` are cancelable — users can call `event.preventDefault()` inside the listener to abort the transition.
