# Canvas Input Component Skill

`<sit-input>` is a single-line form text input supporting all standard HTML input types, with built-in label, hint text, validation feedback, prefix/suffix addons, loading state, and an action slot.
...
- For all standard HTML input types (`text`, `email`, `tel`, `number`, `password`, `search`, `url`) within Canvas forms.

### When NOT to use

- For long-form or multi-line input → use `<sit-textarea>` instead.
- For predefined options → use `<sit-select>`, `<sit-radio>`, or `<sit-checkbox>`.
...
- **Prefix/Suffix**: use `prefix` / `suffix` for text addons (e.g. `$`, `kg`) shown inline inside the input border. For icon addons, use the `action` slot with `<sit-icon-button>` instead.
- **Action slot**: use the `action` slot with `<sit-icon-button>` for icon actions after the field (e.g. toggle password visibility, clear).
- **Debounced input**: for search or filtering, debounce the `sit-input` event to reduce performance load.
...
- **Character limits**: `minlength` and `maxlength` are direct attributes on `<sit-input>`; show counters when limits are enforced. For numeric/date types, use `min` and `max` to constrain the value range.
...
- **Special characters and non-Latin scripts**: `<sit-input>` accepts any Unicode text; validate or sanitise server-side as needed.
...
**Icon action button after the field?** → Use the `action` slot with `<sit-icon-button>`
...
>
  <sit-icon-button slot="action" variant="ghost" name="search"></sit-icon-button>
</sit-input>

<!-- Loading state -->
<sit-input type="text" label="Lookup" loading></sit-input>
...
| Slot | Purpose |
|---|---|
| `action` | Call-to-action element after the input (use `<sit-icon-button>`) |

## Events

| Event | When |
|---|---|
| `sit-change` | Committed value change (on blur / Enter) |
| `sit-input` | Each keystroke that changes the value |
| `sit-focus` | Input gains focus |
| `sit-blur` | Input loses focus |
| `sit-invalid` | Input fails validation |
| `sit-valid` | Input passes validation |
...
2. `prefix` and `suffix` render text inline inside the input border — for icon addons, use the `action` slot with `<sit-icon-button>` instead.
3. `type="number"` with `min` and `max` enforces numeric bounds natively.
4. For reactive frameworks, listen to `sit-input` for immediate value tracking and `sit-change` for committed values.
