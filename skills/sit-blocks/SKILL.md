---
name: "sit-blocks"
description: "Reusable UI blocks for building full Canvas pages and sections. Use this skill whenever the user wants to build pages, design layouts, create sections, or compose multiple sections together. Trigger on ANY mention of: app layout, application shell, page structure, hero sections, CTAs (call-to-action), cards, card grids, feature sections, product benefits, statistics displays, metrics, page headers, page titles, filter interfaces, search filters, data tables, forms, landing pages, dashboards, sidebar navigation, or any major page component — even if they don't name it a 'block'. Also use for: 'I need a filter', 'build a form', 'create a call to action', 'design a landing page', 'show statistics', 'sidebar layout', or similar requests for page-level UI. These are drop-in sections and shell structures, not full pages. Compose them with sit-templates to build complete pages."
metadata:
  author: sit-canvas
  version: "0.0.0"
  audience: external
...
Reusable UI blocks that slot into any page layout. Each block is a self-contained section — drop it into a page template from **[sit-templates](../sit-templates/SKILL.md)** to assemble complete pages without writing layout code from scratch.
...
The **Application Shell** is a special mandatory block — it is the page chrome (`<sit-masthead>`, `<sit-mainnav>`, `<sit-footer>`) that every Canvas page must include. All other blocks are content blocks that slot inside the shell.
...
See **[sit-components](../sit-components/SKILL.md)** for full installation details.
...
|---|---|
| Mandatory page chrome (masthead, mainnav, footer, container) for any Canvas page | [Application Shell](reference/application-shell.md) |
...
**Required for every page.** The application shell wraps all page content with the mandatory Singapore Institute of Technology chrome: `<sit-masthead>`, `<sit-mainnav>`, and `<sit-footer>`. Provides two layout variants — Simple App (`.sit-container`, public-facing) and Sidebar App (`.sit-container-sidebar`, dashboards and internal tools) — with full breakpoint tables and sticky-header patterns.
...
<div class="sit:bg-surface-default sit:min-h-screen">
  <div class="sit:w-container sit:mx-auto sit:py-layout-md">
    ...
    <div class="sit:flex sit:gap-layout-md sit:items-start">

      <!-- Drop the block here -->
      <aside class="sit:shrink-0 sit:w-64">
...
Forms are complex because Canvas components have specific constraints. **Read [reference/form.md](reference/form.md) FIRST** — it contains all grid math, component pairing rules, 5 canonical patterns, and positioning variants.

### Component API Verification
Read the component reference for every `<sit-*>` element you use:

- **Child element naming**: Each component has specific child element names. `<sit-select>` uses `<sit-select-option>` (not `<sit-option>`). `<sit-combo-box>` uses `<sit-combobox-option>`. Always verify in the [sit-components skill](../sit-components/SKILL.md) reference.
- **Slot requirements**: `<sit-file-upload>` slots require text labels (not just icons). Check if the component expects child elements, slot content, or JS properties.
- **Optional properties**: `<sit-stepper>` uses a JS property (`steps` array), not HTML child elements like `<sit-stepper-item>`. Verify which components accept attributes vs. JS properties.

### Form-Specific Constraints

From [reference/form.md](reference/form.md):

- Forms take exactly **8 columns** within `.sit-container`
...
Users are free to design their own blocks with full creative latitude — layout, composition, and visual hierarchy are all open. The only constraint is that every block must stay within the Canvas system rails:

| Requirement | How |
|---|---|
| **UI components** | Use `<sit-*>` web components. Do not reach for plain HTML equivalents when a Canvas component exists (e.g. use `<sit-badge>`, not a hand-rolled `<span>` chip). |
| **Styling** | Use `sit:` Tailwind utilities exclusively for colours, spacing, typography, and layout. Do not write arbitrary CSS values that duplicate what the design token system already expresses. |
| **Typography** | Use semantic role tokens (`sit:text-heading-md`, `sit:text-body-md`, `sit:text-overline-md`, etc.) paired with matching weight, line-height, and tracking tokens. Do not use raw scale tokens (`sit:text-base`, `sit:text-sm`) which are not part of the public API. |
| **Icons** | Use `<sit-icon name="...">` exclusively. Do not embed raw SVG or third-party icon libraries. |
| **External inspiration** | Fine to reference sites like [shadcnblocks.com](https://www.shadcnblocks.com/blocks/about), Tailwind UI, or any other design gallery for layout ideas — but always re-implement using Canvas components and tokens, not the source site's CSS or component library. |
...
1. **LOAD `sit-utilities` skill** — You need the complete utility class reference
2. **LOAD `sit-components` skill** — You need component APIs and child element names
3. **Then READ `reference/custom-block-rules.md`** in this skill

**STYLING RULE (applies to ALL blocks):**
- Use `sit:` Tailwind utilities EXCLUSIVELY for spacing, colors, typography, layout
- Use `<sit-*>` components EXCLUSIVELY for UI elements
- **NEVER use inline `<style>` blocks** to replicate design tokens (e.g., `margin: 24px`, `var(--sit-*)`)
...
1. Load `sit-utilities` skill
2. Load `sit-components` skill
3. Load this skill
...
1. **Every page must have the Application Shell** — `<sit-masthead>`, `<sit-mainnav>`, and `<sit-footer>` are mandatory on every Canvas page. Read **[reference/application-shell.md](reference/application-shell.md)** for layout patterns and container classes.
2. When a user asks for a filtered list page, combine the Filter Sidebar block with the List Page template from **[sit-templates](../sit-templates/SKILL.md)**.
