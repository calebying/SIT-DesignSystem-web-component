...
import type { SitTabGroup, SitTabPanel, SitTab } from "../src/components";
...
describe("<sit-tab>", () => {
  ...
  it("should render default tab", async () => {
    const el = await fixture<SitTab>(html`<sit-tab>Test</sit-tab>`);
  ...
  it("should disable tab by attribute", async () => {
    const el = await fixture<SitTab>(html` <sit-tab disabled>Test</sit-tab> `);
...
describe("<sit-tab-panel>", () => {
  ...
  it("passed id should be used", async () => {
    const el = await fixture<SitTabPanel>(html` <sit-tab-panel id="test-id">Test</sit-tab-panel> `);

    expect(el.id).to.equal("test-id");
  });
});

describe("<sit-tab-group>", () => {
  it("renders", async () => {
    const tabGroup = await fixture<SitTabGroup>(html`
      <sit-tab-group>
        <sit-tab slot="nav" panel="general">General</sit-tab>
        <sit-tab-panel name="general">This is the general tab panel.</sit-tab-panel>
      </sit-tab-group>
    `);
  ...
  it("displays all tabs", async () => {
    const tabGroup = await fixture<SitTabGroup>(html`
      <sit-tab-group>
        <sit-tab slot="nav" panel="general" data-testid="general-tab-header">General</sit-tab>
        <sit-tab slot="nav" panel="disabled" disabled data-testid="disabled-tab-header">Disabled</sit-tab>
        <sit-tab-panel name="general">This is the general tab panel.</sit-tab-panel>
        <sit-tab-panel name="disabled">This is a disabled tab panel.</sit-tab-panel>
      </sit-tab-group>
    `);
  ...
  propsToForwardToChild.forEach(({ name, defaultValue, changedValue }) => {
    it(`change of ${name} prop updates its sit-tab children`, async () => {
      const tabGroup = await fixture<SitTabGroup>(html`
        <sit-tab-group>
          <sit-tab slot="nav" panel="general">General</sit-tab>
          <sit-tab slot="nav" panel="custom">Custom</sit-tab>
...
const expectHeaderToBeVisible = (container: HTMLElement, dataTestId: string): void => {
  const generalHeader = container.querySelector<SitTabGroup>(`[data-testid="${dataTestId}"]`);
...
const waitForHeaderToBeActive = async (container: HTMLElement, headerTestId: string): Promise<SitTab> => {
  const headerTab = container.querySelector<SitTab>(`[data-testid="${headerTestId}"]`);
  await waitUntil(() => {
...
const expectCustomTabToBeActiveAfter = async (tabGroup: SitTabGroup, action: () => Promise<void>): Promise<void> => {
...
describe("tab selection", () => {
  it("should render default tab", async () => {
    ...
    const shadowTab = SitTab?.shadowRoot?.querySelector('[data-testid="inner-tab"]');
  ...
  it("does not change if a disabled tab is clicked", async () => {
    const tabGroup = await fixture<SitTabGroup>(html`
      <sit-tab-group>
        <sit-tab slot="nav" panel="general" data-testid="general-header">General</sit-tab>
        <sit-tab slot="nav" panel="disabled" data-testid="disabled-header" disabled>disabled</sit-tab>
        <sit-tab-panel name="general" data-testid="general-tab-content">This is the general tab panel.</sit-tab-panel>
        <sit-tab-panel name="disabled">This is the disabled tab panel.</sit-tab-panel>
      </sit-tab-group>
    `);

    const disabledHeader = tabGroup.querySelector<SitTab>('[data-testid="disabled-header"]');
  ...
  it("selects a tab by using the arrow keys", async () => {
    const tabGroup = await fixture<SitTabGroup>(html`
      <sit-tab-group>
        <sit-tab slot="nav" panel="general" data-testid="general-header">General</sit-tab>
        <sit-tab slot="nav" panel="custom" data-testid="custom-header">Custom</sit-tab>
        <sit-tab-panel name="general">This is the general tab panel.</sit-tab-panel>
        <sit-tab-panel name="custom" data-testid="custom-tab-content">This is the custom tab panel.</sit-tab-panel>
      </sit-tab-group>
    `);
  ...
  it("does not allow selection of disabled tabs with arrow keys", async () => {
    const tabGroup = await fixture<SitTabGroup>(html`
      <sit-tab-group>
        <sit-tab slot="nav" panel="general" data-testid="general-header">General</sit-tab>
        <sit-tab slot="nav" panel="disabled" disabled>Disabled</sit-tab>
        <sit-tab-panel name="general" data-testid="general-tab-content">This is the general tab panel.</sit-tab-panel>
        <sit-tab-panel name="disabled">This is the custom tab panel.</sit-tab-panel>
      </sit-tab-group>
    `);
