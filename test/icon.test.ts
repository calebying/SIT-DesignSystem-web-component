...
import { SitIcon } from "../src/components";
import "./sit-web-component";

describe("<sit-icon>", () => {
  it("has default properties", async () => {
    const el = await fixture<SitIcon>(html`<sit-icon></sit-icon>`);
  ...
  it("handles invalid icon name gracefully", async () => {
    ...
    const el = await fixture<SitIcon>(html`<sit-icon name="invalid-icon"></sit-icon>`);
  ...
  it("should render icon when icon is available", async () => {
    ...
    const el = await fixture<SitIcon>(html`<sit-icon name="calendar"></sit-icon>`);
