...
import type { SitToast } from "../src/components";
import sinon from "sinon";

describe("SitToast component", () => {
  ...
  it("when show set is true, sit-show and sit-after-show events are emitted", async () => {
    const el = await fixture<SitToast>(html`<sit-toast></sit-toast>`);
  ...
  it("when show set is false, sit-hide and sit-after-hide events are emitted", async () => {
    const el = await fixture<SitToast>(html`<sit-toast show></sit-toast>`);
  ...
  it("when autohide is true, toast disappears after the specified delay of 100ms", async () => {
    const el = await fixture<SitToast>(html`<sit-toast show autohide delay="100"></sit-toast>`);
  ...
  it("class .d-none absent the action slot when action slot is present is true", async () => {
    const el = await fixture<SitToast>(html`<sit-toast show><span slot="action">Action</span></sit-toast>`);
    await elementUpdated(el);
